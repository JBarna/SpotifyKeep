...
function setupTray() {
  ...
  contextMenu = Menu.buildFromTemplate([
      ...
      submenu: [
        {
          ...
          type: "checkbox",
          label: "Notifications have sound",
        ...
        { type: "separator" },
        {
          type: "normal",
          label: "Change Hotkey",
          sublabel: `Current: ${Lib.Hotkey.getHotkey()}`,
    ...
    { type: "separator" },
    {
      ...
      icon: getIcon("about"),
      click: () =>
        opener("https://github.com/JBarna/SpotifyKeep/blob/master/readme.md"),
    },
    {
      label: "Report an Issue",
      icon: getIcon("error"),
      click: () => opener("https://github.com/JBarna/SpotifyKeep/issues"),
    },
    { type: "separator" },
  ...
  if (!~~(Math.random() * 15)) {
    contextMenu.insert(
      ...
      new MenuItem({
        label: "China #1",
        icon: getIcon("china"),
        click: (menuItem) => (menuItem.visible = false),
      })
    );
  }
...
function getIcon(iconFileName) {
  var natImg = nativeImage
    .createFromPath(Lib.fs.getFullPath(`/icons/${iconFileName}.png`))
    .resize({ width: 25 });
