...
var refreshPromise, OAuthPromiseResolve;
...
function requestAccess() {
  ...
  requestUrl.query = {
    ...
    scope:
      "user-library-modify user-library-read playlist-modify-public playlist-read-private playlist-modify-private user-read-currently-playing",
  };
...
function refreshToken() {
  ...
  var url_options = buildUrlOptions(),
    dataToSend =
      ...
      Lib.State.get(Lib.State.keys.REFRESH_TOKEN);

  return Lib.HttpsHelper.send(url_options, dataToSend).then(saveToken);
}
...
function getBearerToken() {
  ...
  if (!accessToken) {
    if (!OAuthPromiseResolve) return beginOAuth();
    ...
    } else return Promise.resolve(accessToken);
  }
}
