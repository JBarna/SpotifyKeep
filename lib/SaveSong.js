...
async function getCurrentSong() {
    let url_options, song;
    
    url_options = Object.assign({},
        await Lib.HttpsHelper.buildUrlOptions(),
        {path: '/v1/me/player/currently-playing'});

    song = await Lib.HttpsHelper.send(url_options);
    console.debug('Current Song', song);
    if (song === 204 || !song.is_playing || !song.item) {
...
function saveSongToPlaylist(target) {
    return getCurrentSong()
        .then(song => {
            return Lib.HttpsHelper.buildUrlOptions().then(urlOptions => {
                urlOptions.method = 'POST';
                urlOptions.headers['Content-Type'] = "application/json";
                urlOptions.path = `/v1/playlists/${target}/tracks`;
    
                return Lib.HttpsHelper.send(urlOptions, JSON.stringify({uris: [song.uri]}))
            }).then(() => {
                const playlists = Lib.State.get(Lib.State.keys.WRITABLE_PLAYLISTS)
                const targetPlaylist = playlists.find(list => list.id === target)
                song.targetPlaylistName = targetPlaylist.name
                return song
            })
        }).catch(err => {
            console.debug("error saving song to playlist:", err);
            return {err};
        });
}

function saveSong(target){
    var saveSongPromise;
    console.debug('Saving Song');

    if (target !== "Liked Songs") {
        return saveSongToPlaylist(target)
    }
