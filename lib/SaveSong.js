...
async function getCurrentSong() {
  ...
  url_options = Object.assign({}, await Lib.HttpsHelper.buildUrlOptions(), {
    ...
    return Lib.HttpsHelper.send(urlOptions).then((songListSection) => {
      ...
      if (updatedSongIndex === -1) {
        ...
        if (updatedSongIndex + queries.offset !== songIndex) {
          console.debug(
            "Existing index does not match",
            songIndex,
            updatedSongIndex + queries.offset
          );
...
function saveSong() {
  ...
  return Promise.all([
    getCurrentSong(),
    Lib.State.get(Lib.State.keys.SAVED_SONGS_LIST),
  ])
