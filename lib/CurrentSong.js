...
const childProcess = require('child_process'),
    {getTrack, isRunning, getState} = require('spotify-node-applescript'),
    EXEC_STRING = 'tasklist /fi "imagename eq spotify.exe" /fo list /v',
    ERR_SPOTIFY_NOT_RUNNING = "Spotify is not running",
    ERR_NO_SONG_PLAYING = 'No song is playing';
    ...
    return new Promise((resolve, reject) => {
        if (process.platform === "win32") {
            ...
            if (windowTitle.indexOf("INFO") === 0)
                reject(ERR_SPOTIFY_NOT_RUNNING);
            ...
            isRunning().then(running => running || Promise.reject(ERR_SPOTIFY_NOT_RUNNING))
                .then(getState).then(state => state.state === 'playing' ? state : Promise.reject(ERR_NO_SONG_PLAYING))
                .then(getTrack).then(track => {
                    track.id = track.id.split(':')[2];
                    resolve(track);
                }).catch(reject);
        }
    });
