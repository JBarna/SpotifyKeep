...
function GetWritablePlaylists() {
    ...
    return Lib.HttpsHelper.getList({
        ...
        } // we can write to a playlist if we own it or if it's marked collaborative
    }).then(playlists => playlists.filter(list => list.owner.id === Lib.State.get(Lib.State.keys.USER_ID) || list.collaborative))
    .then(writablePlaylists => {
        // cache locally so we'll have the name when we save
        Lib.State.set(Lib.State.keys.WRITABLE_PLAYLISTS, writablePlaylists.map(list => ({ id: list.id, name: list.name })))
        return writablePlaylists
    })
}

function BackupToKeep() {
    return getUserPlaylists()
        ...
        .catch(console.debug.bind(null, 'Error routinely checking saved songs and keep playlist'));
}
