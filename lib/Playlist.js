...
function Playlist() {
  return getUserPlaylists()
    ...
    .then((keepPlaylist) =>
      Promise.all([keepPlaylist, getKeepPlaylistTracks(keepPlaylist)])
    )
    .then(saveSongsToKeepPlaylist)
    .catch(
      console.debug.bind(
        null,
        "Error routinely checking saved songs and keep playlist"
      )
    );
}
...
function getKeepPlaylist(userPlaylists) {
  ...
  if (keepPlaylist) return keepPlaylist;

  console.debug("creating keep playlist");
  return Lib.HttpsHelper.buildUrlOptions().then(
    (createKeepPlaylistUrlOptions) => {
      createKeepPlaylistUrlOptions.headers["Content-Type"] = "application/json";
      createKeepPlaylistUrlOptions.method = "POST";
      createKeepPlaylistUrlOptions.path = `/v1/users/${Lib.State.get(
        Lib.State.keys.USER_ID
      )}/playlists`;
      var playlistData = {
        name: "Keep",
        description:
          "A backup playlist created and maintained by SpotifyKeep. Find out more at www.github.com/JBarna/SpotifyKeep",
      };
      return Lib.HttpsHelper.send(
        createKeepPlaylistUrlOptions,
...
function getKeepPlaylistTracks(keepPlaylist) {
  ...
  return Lib.HttpsHelper.buildUrlOptions().then((snapshotUrlOptions) => {
    snapshotUrlOptions.path = `/v1/users/${Lib.State.get(
      Lib.State.keys.USER_ID
    )}/playlists/${keepPlaylist.id}?fields=snapshot_id`;
    return Lib.HttpsHelper.send(snapshotUrlOptions).then((response) => {
      console.debug("keep playlist snapshot id", response.snapshot_id);
      if (
        response.snapshot_id ===
        Lib.State.get(Lib.State.keys.KEEP_PLAYLIST_SNAPSHOT)
      )
      ...
      Lib.State.set(
        Lib.State.keys.KEEP_PLAYLIST_SNAPSHOT,
        response.snapshot_id
      );
      return Lib.HttpsHelper.getList({
        path: `/v1/users/${Lib.State.get(Lib.State.keys.USER_ID)}/playlists/${
          keepPlaylist.id
        }/tracks`,
...
function saveSongsToKeepPlaylist([keepPlaylist, keepPlaylistTracks]) {
  ...
  tracksToKeep = savedTracks
    .filter((track) => keepPlaylistTracks.indexOf(track) === -1)
  ...
  return Promise.all(
    tracksToKeepArray.map((tracks) => {
      return Lib.HttpsHelper.buildUrlOptions().then((keepUrlOptions) => {
        ...
        keepUrlOptions.path = `/v1/users/${Lib.State.get(
          Lib.State.keys.USER_ID
        )}/playlists/${keepPlaylist.id}/tracks`;
